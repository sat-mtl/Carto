...
## If true, the subviewports of each cameras will be dynamically resized to match
## the main viewport's largest dimension.
@export var auto_resize_subviewport: bool = false:
	set(value):
		auto_resize_subviewport = value
		set_auto_resize_callback()
...
func adapt_subviewports_sizes() -> void:
	mat.set_shader_parameter("resolution", get_viewport().size)
	var biggest_square_size: Vector2i = Vector2i.ONE*(max(get_viewport().size[0], get_viewport().size[1]))
	for viewport in viewports:
		viewport.size = biggest_square_size

func set_auto_resize_callback() -> void:
	if not get_viewport():
		return
	if auto_resize_subviewport:
		get_viewport().size_changed.connect(adapt_subviewports_sizes)
	elif get_viewport().size_changed.is_connected(adapt_subviewports_sizes):
		get_viewport().size_changed.disconnect(adapt_subviewports_sizes)

func _ready() -> void:
	var initial_cull_mask = cull_mask
	set_auto_resize_callback()
	...
	for i in num_cameras:
		...
		camera.cull_mask = initial_cull_mask - render_layer
		#camera.set_cull_mask_value(render_layer, false)
