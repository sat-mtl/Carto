...
@export
var _selection_box_color := Color(1.0, .5, 0,0)
## The color of the AABB surrounding the target nodes.
...
func process_input(event: InputEvent):
	...
	if !visible:
		...
		# not the cleanest place to put this but it works.
...
func _update_transform_gizmo_view() -> void:
	...
	# Selection box
	if !editing and !calculate_bounds:
		return
	for key in _selections:
...
func _transform_gizmo_apply(node : Node3D, transform : Transform3D, local : bool) -> void:
	# bugfix so that scaling doesn't also apply position and rotation.
	# The other modes probably needs something equivalent.
	if _edit.mode == TransformMode.SCALE:
		node.scale = transform.basis.get_scale()
		return
